...
def load_sgdp_metadata(metadata_file='SGDP_metadata.279public.21signedLetter.44Fan.samples.txt', verbose=True):
    ...
    if verbose:
        print(
            f"[METADATA] Loading SGDP metadata from {metadata_file}...", flush=True)
    ...
    for encoding in encodings_to_try:
        try:
            with open(metadata_file, encoding=encoding) as f:
                ...
                for line in f:
                    if line.strip():
                        ...
                        id_columns = [
                            'Sample_ID', 'Sample_ID(Aliases)', 'SGDP_ID', 'Illumina_ID']
            ...
            if verbose:
                print(
                    f"[METADATA] Successfully read file with {encoding} encoding", flush=True)
                print(
                    f"[DEBUG] Created metadata entries for {len(metadata)} sample IDs", flush=True)
                # Show some examples
                brahui_samples = [k for k in metadata.keys(
                ) if 'Brahui' in k or 'S_Brahui' in k]
                if brahui_samples:
                    print(
                        f"[DEBUG] Brahui samples found: {brahui_samples[:5]}", flush=True)
                hezhen_samples = [k for k in metadata.keys(
                ) if 'Hezhen' in k or 'S_Hezhen' in k]
                if hezhen_samples:
                    print(
                        f"[DEBUG] Hezhen samples found: {hezhen_samples[:5]}", flush=True)
            break  # Success, exit the encoding loop

        except UnicodeDecodeError:
            if verbose and encoding == encodings_to_try[-1]:
                print(
                    f"[WARNING] All encodings failed. Trying with error handling...", flush=True)
            continue
        except Exception as e:
            if verbose:
                print(
                    f"[WARNING] Could not load metadata with {encoding}: {e}", flush=True)
    ...
    if not metadata:
        try:
            ...
            if verbose:
                print(
                    f"[WARNING] Could not load metadata even with error handling: {e}", flush=True)
            return {}

    if verbose:
        print(
            f"[METADATA] Loaded metadata for {len(metadata)} samples", flush=True)
    return metadata
...
def write_23andme_format_files(sample_data, metadata, output_dir='sgdp', verbose=True):
    ...
    if verbose:
        print(
            f"[OUTPUT] Writing 23andMe format files to {output_dir}/...", flush=True)
    ...
    for sample_id in sample_data.keys():
        if sample_id in metadata:
            ...
            if verbose and 'Unknown' in [pop]:
                print(
                    f"[DEBUG] Sample {sample_id} has population {pop}", flush=True)
        else:
            pop = 'Unknown'
            if verbose:
                print(
                    f"[DEBUG] Sample {sample_id} not found in metadata - assigning to Unknown", flush=True)
        samples_by_pop[pop].append(sample_id)

    if verbose:
        print(
            f"[OUTPUT] Found {len(samples_by_pop)} populations: {list(samples_by_pop.keys())}", flush=True)
    ...
    for pop, sample_ids in samples_by_pop.items():
        ...
        if verbose:
            print(
                f"[OUTPUT] Writing {len(sample_ids)} samples for population {pop}...", flush=True)
        ...
        for idx, sample_id in enumerate(sample_ids, 1):
            ...
            with open(sample_file, 'w') as f:
                ...
                f.write(
                    f"# Coordinates: {meta.get('latitude', '')},{meta.get('longitude', '')}\n")
                f.write(
                    f"# Contributor: {meta.get('contributor', 'Unknown')}\n")
            ...
            if verbose and (idx % 50 == 0 or idx == len(sample_ids)):
                print(
                    f"    [PROGRESS] {idx}/{len(sample_ids)} samples written for {pop}", flush=True)

    if verbose:
        print(
            f"[OUTPUT] Complete! {total_samples_written} sample files written to {output_dir}/", flush=True)
    return output_dir


def batch_query_snps_to_23andme(csv_file='merged_snps.csv', metadata_file='SGDP_metadata.279public.21signedLetter.44Fan.samples.txt', output_dir='sgdp', verbose=True):
    """
    ...
    if verbose:
        print(
            f"[STARTUP] Found SNPs on {len(snps_by_chrom)} chromosomes: {list(snps_by_chrom.keys())}", flush=True)
    ...
    for chrom, snplist in snps_by_chrom.items():
        ...
        if not chrom_key:
            if verbose:
                print(
                    f"[WARN] Skipping unsupported chromosome {chrom}", flush=True)
            continue
        ...
        if verbose:
            print(
                f"[INFO] ({chrom_idx}/{total_chroms}) Querying {BCF_URL} for {len(snplist)} SNPs on chr{chrom}", flush=True)
        ...
        try:
            ...
            if verbose:
                print(
                    f"    [CONNECT] Successfully connected to chr{chrom} BCF", flush=True)
        except Exception as e:
            if verbose:
                print(
                    f"[ERROR] Could not open remote BCF for chr{chrom}: {e}", flush=True)
            continue
        ...
        for snp_idx, snp in enumerate(snplist, 1):
            ...
            if verbose and (snp_idx == 1 or snp_idx % 10 == 0 or snp_idx == total_snps):
                ...
                print(
                    f"    [PROGRESS] chr{chrom}: SNP {snp_idx}/{total_snps} (pos {pos}, rsid {rsid}) - {rate:.1f} SNPs/sec, ETA: {eta:.0f}s", flush=True)

            for rec in bcf.fetch(chrom, pos-1, pos):
                found = True
                if verbose and snp_idx % 25 == 0:
                    print(
                        f"        [SAMPLES] Processing {len(rec.samples)} samples for SNP {rsid}", flush=True)
        ...
        if verbose:
            ...
            print(
                f"    [COMPLETE] chr{chrom} finished in {chrom_elapsed:.1f}s ({len(snplist)} SNPs)", flush=True)
    ...
    output_dir = write_23andme_format_files(
        sample_data, metadata, output_dir, verbose)
